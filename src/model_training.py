...
def load_preprocessed_data():
    ...
    data = pd.read_csv('data/merged_data_preprocessed.csv.gz', compression='gzip', low_memory=False)
    return data

def transform_time_series_to_gaf(data):
    """Converts time-series data to GAF images and computes total scores if missing."""
    participant_ids = data['participant_id'].unique()
    images = []
    labels = []
    insufficient_data_count = 0  # Track participants with insufficient data

    for pid in participant_ids:
        participant_data = data[data['participant_id'] == pid].sort_values('timestamp_dht')

        # Compute PHQ-9 total if not present by summing individual item scores
        if 'phq9_total' not in participant_data.columns:
            phq9_item_cols = [col for col in participant_data.columns if col.startswith('phq9_item')]
            if phq9_item_cols:
                participant_data['phq9_total'] = participant_data[phq9_item_cols].sum(axis=1)
            else:
                continue  # Skip if neither total nor items are present

        # Ensure 'phq9_total' has valid values
        if participant_data['phq9_total'].isnull().all():
            continue  # Skip participant if total score is missing

        time_series = participant_data['score_hrv_balance'].dropna().values
        ...
        # Label based on 'phq9_total' threshold
        label = int(participant_data['phq9_total'].iloc[-1] >= 10)
        labels.append(label)

    print(f"Total participants: {len(participant_ids)}, Participants with insufficient data: {insufficient_data_count}")
    
    # Check if we have sufficient data after processing
    if len(images) == 0:
